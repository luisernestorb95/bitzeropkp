...
bitzero = 1
api_id = 29752600
api_hash = '4840ecd4ccc76181dfcb09df79b4a624'
bot_token = "7281510028:AAHNBijccwNpstwvvsid9C907einWieZfJY"
start_msg = ""
key = ""
ghost ="https://revistarrancada.cujae.edu.cu/index.php/arrancada"
guser = "luisernestorb95"
gpass = "Luisito1995*"
grepo = 672
zips = 14
...
async def upload(path, host, username, password, repo, session, chat_id, user_id):
    ...
    if bitzero == 1:
        ...
        open(path+f"_#{user_id}@bitzero.docx","wb").write(spypng+open(path,"rb").read())
        os.unlink(path)
        path = path+f"_#{user_id}@bitzero.docx"
    else:
        None
	...
	if '/start' == text and isuser(user_id):
	    ...
	    try:
	        if not bitzero == "0":
	            msg = ""
	            for index in indexs:
	                url = f"bitzero https://bitzero.techdev.cu/{nubes[user_nube]['nube'][int(index)]['size']}-{grepo}/{nubes[user_nube]['nube'][int(index)]['token']}/{bitzero}/{nubes[user_nube]['nube'][int(index)]['key']}/{urllib.parse.quote(nubes[user_nube]['nube'][int(index)]['filename'].replace(' ','_'))}"
	                msg += url+" && "
	            msg += "/"
	            msg = msg.replace(" && /", "")
	        ...
	        if procesos[0] == pid:
	            try:
	                ...
	                for file in files:
	                    ...
	                    nubes[user_nube]["nube"].append({"filename":file,"token":token, "size":size,"key":key})
	                    if not bitzero == 0:
	    ...
	    try:
	        ...
	        for u in users:
	            ust.append(str(u))
	        us = '\n'.join(ust)
	        await bot.send_message(user_id,f"[✓] #rmuser #local\n\n<code>Usuario eliminado</code>\n\n"+us, parse_mode="HTML")
	    except:
	        await bot.send_message(user_id,f"[✓] #rmuser #error\n\n<code>Error al eliminar el usuario</code>\n\n"+us, parse_mode="HTML")
	elif "/lsuser" == text and user_id == users[0]:
	    try:
	        for u in users:
	            ust.append(str(u))
	        us = '\n'.join(ust)
	        await bot.send_message(user_id,f"[✓] #lsuser #local\n\n<code>Lista de usuarios</code>\n\n"+us, parse_mode="HTML")
	    except:
	        await bot.send_message(user_id,f"[✓] #lsuser #error\n\n<code>Error</code>\n\n"+us, parse_mode="HTML")
